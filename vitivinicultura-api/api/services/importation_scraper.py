...
class ImportationScraper():
    ...
    def get_year(self):
        try:
            response = requests.get(self.base_url + '?opcao=opt_05', timeout=10)
    ...
    def importation_table(self):
        ...
        for year in self.years:
            for subop in range(1, 6):
                ...
                url = f"{self.base_url}?subopcao={suboption}&opcao=opt_05&ano={year}"
                try:
    ...
    def save_df(self, df):
        ...
        filepath = os.path.join(path, 'tabela_importacao.csv')
    ...
    def get_json(self):
        ...
        try:
            ...
            try:
                df_fallback = pd.read_csv("vitivinicultura-api/data/tabela_importacao.csv")
